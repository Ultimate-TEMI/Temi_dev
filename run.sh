...
tmux new-window -t $SESSION -n "person_detection"
tmux send-keys -t $SESSION:2 "ros2 run person_detection_ros person_detection_node --ros-args -p use_sim_time:=false" C-m

tmux new-window -t $SESSION -n "pdaf_tracker"
tmux send-keys -t $SESSION:3 "ros2 launch pdaf_tracker track.launch.py" C-m
