...
tmux send-keys -t $SESSION:2 "python3 /home/temi/Temi_DBInfo/DB_App/app.py" C-m

# tmux new-window -t $SESSION -n "detection"
# tmux send-keys -t $SESSION:3 "ros2 launch person_detection_ros detect.launch.py use_sim_time:=false use_rviz:=false namespace:=temi" C-m

# tmux new-window -t $SESSION -n "tracker"
# tmux send-keys -t $SESSION:4 "ros2 launch pdaf_tracker track.launch.py namespace:=temi use_sim_time:=false" C-m
